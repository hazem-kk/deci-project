...
let nearestStopId = 'tahrir-square';
...
/* ============================= GOOGLE MAP ============================= */
let googleMap=null,googleMapReady=false,googleUserMarker=null,googleStopMarkers=[],googleSearchMarker=null,googleRouteLine=null,googleMapInitPromise=null,lastGoogleSearch=null;
const STOP_COORDS={
  'tahrir-square':{lat:30.0444,lng:31.2357},'ramses':{lat:30.0626,lng:31.2461},'zamalek':{lat:30.0626,lng:31.2197},
  'nasr-city':{lat:30.0511,lng:31.3656},'maadi':{lat:29.9602,lng:31.2569}
};
function makeMapPin(label,variant='stop'){const el=document.createElement('div');el.className='busly-map-pin busly-map-pin--'+variant;el.innerHTML='<span></span><b>'+escapeHtml(String(label))+'</b>';return el;}
function clearGoogleMarkers(){googleStopMarkers.forEach(m=>m.map=null);googleStopMarkers=[];if(googleUserMarker){googleUserMarker.map=null;googleUserMarker=null;}if(googleSearchMarker){googleSearchMarker.map=null;googleSearchMarker=null;}if(googleRouteLine){googleRouteLine.setMap(null);googleRouteLine=null;}}
async function initGoogleMap(){
  if(googleMapReady)return googleMap;if(googleMapInitPromise)return googleMapInitPromise;
  const key=window.BUSLY_GOOGLE_MAPS_API_KEY,status=$('#mapStatusPill');
  if(!key||key==='YOUR_GOOGLE_MAPS_API_KEY'){
    status.innerHTML='<span class="map-status-dot map-status-dot--warn"></span><span>Google Maps key required</span>';
    const mapEl=document.getElementById('googleMap');
    if(mapEl) mapEl.innerHTML='<div class="map-key-message"><div class="map-key-icon">⌖</div><strong>Connect Google Maps</strong><span>Paste your Google Maps browser API key into <b>index.html</b> to activate the live map and place search.</span></div>';
    return null;
  }
  googleMapInitPromise=new Promise((resolve,reject)=>{
    const existing=document.querySelector('script[data-busly-google-maps]');
    if(existing){
      if(window.google?.maps) return resolve();
      existing.addEventListener('load',()=>resolve(),{once:true});existing.addEventListener('error',reject,{once:true});return;
    }
    window.__buslyGoogleMapsReady=()=>resolve();
    const script=document.createElement('script');
    script.src='https://maps.googleapis.com/maps/api/js?key='+encodeURIComponent(key)+'&loading=async&libraries=places,marker&callback=__buslyGoogleMapsReady&v=weekly';
    script.async=true;script.defer=true;script.dataset.buslyGoogleMaps='true';script.onerror=reject;document.head.appendChild(script);
  }).then(async()=>{
    await Promise.all([google.maps.importLibrary('maps'),google.maps.importLibrary('marker')]);
    const {Map,InfoWindow}=await google.maps.importLibrary('maps');
    googleMap=new Map(document.getElementById('googleMap'),{center:{lat:30.0444,lng:31.2357},zoom:12,mapId:'DEMO_MAP_ID',mapTypeControl:false,streetViewControl:false,fullscreenControl:false,clickableIcons:true,gestureHandling:'greedy',styles:[
      {elementType:'geometry',stylers:[{color:'#ffffff'}]},{elementType:'labels.text.fill',stylers:[{color:'#6d28d9'}]},{elementType:'labels.text.stroke',stylers:[{color:'#ffffff'}]},
      {featureType:'administrative',elementType:'geometry.stroke',stylers:[{color:'#c4b5fd'}]},{featureType:'road',elementType:'geometry',stylers:[{color:'#f5f3ff'}]},
      {featureType:'road',elementType:'geometry.stroke',stylers:[{color:'#ddd6fe'}]},{featureType:'road',elementType:'labels.text.fill',stylers:[{color:'#7c3aed'}]},
      {featureType:'water',elementType:'geometry',stylers:[{color:'#faf7ff'}]},{featureType:'poi',elementType:'geometry',stylers:[{color:'#ffffff'}]},{featureType:'poi',elementType:'labels.text.fill',stylers:[{color:'#8b5cf6'}]},
      {featureType:'transit',elementType:'geometry',stylers:[{color:'#ede9fe'}]},{featureType:'transit',elementType:'labels.text.fill',stylers:[{color:'#6d28d9'}]}]});
    googleInfoWindow=new InfoWindow();googleMapReady=true;status.innerHTML='<span class="map-status-dot"></span><span>Google Maps · Cairo</span>';renderGoogleMap();return googleMap;
  }).catch(err=>{console.error('Google Maps failed to load:',err);status.innerHTML='<span class="map-status-dot map-status-dot--warn"></span><span>Map could not load</span>';showToast('Google Maps could not load. Check the API key and enabled APIs.',false);return null;});
  return googleMapInitPromise;
}
async function renderGoogleMap(){
  if(!googleMapReady||!googleMap)return;const {AdvancedMarkerElement}=await google.maps.importLibrary('marker');clearGoogleMarkers();
  const bounds=new google.maps.LatLngBounds(),selectedStop=STOP_COORDS[nearestStopId]||STOP_COORDS['tahrir-square'];
  const userPos=userMarkerPos&&userMarkerPos.lat?userMarkerPos:{lat:selectedStop.lat+0.0038,lng:selectedStop.lng-0.005};
  googleUserMarker=new AdvancedMarkerElement({map:googleMap,position:userPos,title:'Your location',content:makeMapPin('You','you')});bounds.extend(userPos);
  STOPS.forEach(stop=>{const coord=STOP_COORDS[stop.id];if(!coord)return;const marker=new AdvancedMarkerElement({map:googleMap,position:coord,title:stop.name,content:makeMapPin(stop.name.replace(' Stop',''),stop.id===nearestStopId?'selected':'stop')});marker.addListener('click',()=>selectStop(stop.id,false));googleStopMarkers.push(marker);bounds.extend(coord);});
  const destinationCoord=lastGoogleSearch?.location||(currentDestination?STOP_COORDS[currentDestination.stopId]:null);
  if(destinationCoord){googleSearchMarker=new AdvancedMarkerElement({map:googleMap,position:destinationCoord,title:lastGoogleSearch?.name||currentDestination?.name||'Destination',content:makeMapPin(lastGoogleSearch?.name||currentDestination?.name||'Destination','destination')});bounds.extend(destinationCoord);$('#mapPlaceName').textContent=lastGoogleSearch?.name||currentDestination?.name||'Destination';$('#mapPlaceAddress').textContent=lastGoogleSearch?.address||'Selected on the map';$('#mapPlaceCard').hidden=false;}
  googleRouteLine=new google.maps.Polyline({map:googleMap,path:destinationCoord?[userPos,destinationCoord]:[userPos,selectedStop],geodesic:true,strokeColor:'#a855f7',strokeOpacity:.95,strokeWeight:4,icons:[{icon:{path:'M 0,-1 0,1',strokeOpacity:1,scale:3},offset:'0',repeat:'14px'}]});
  googleMap.fitBounds(bounds,{top:80,right:40,bottom:80,left:40});if(googleMap.getZoom()>15)googleMap.setZoom(15);
}
function renderMap(){initGoogleMap().then(()=>renderGoogleMap());}
async function searchGooglePlace(query){
  const value=query.trim();if(!value){showToast('Type a place to search on the map',false);return;}const map=await initGoogleMap();if(!map)return;
  const {Place}=await google.maps.importLibrary('places');
  try{
    const {places}=await Place.searchByText({textQuery:value+', Cairo, Egypt',fields:['displayName','formattedAddress','location','viewport'],locationBias:map.getCenter(),language:'en',region:'eg',maxResultCount:5});
    if(!places||!places.length){showToast('No place found for "'+value+'"',false);return;}
    const place=places[0];lastGoogleSearch={name:place.displayName||value,address:place.formattedAddress||'Cairo, Egypt',location:place.location};
    $('#mapSearchInput').value=lastGoogleSearch.name;$('#mapPlaceName').textContent=lastGoogleSearch.name;$('#mapPlaceAddress').textContent=lastGoogleSearch.address;$('#mapPlaceCard').hidden=false;
    map.setCenter(place.location);map.setZoom(16);renderGoogleMap();showToast('Showing "'+lastGoogleSearch.name+'" on Google Maps');
  }catch(error){console.error('Place search failed:',error);showToast('Place search failed. Check that Places API (New) is enabled.',false);}
}
$('#mapSearchBtn').addEventListener('click',()=>searchGooglePlace($('#mapSearchInput').value));
$('#mapSearchInput').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();searchGooglePlace(e.currentTarget.value);}});
$('#mapResetBtn').addEventListener('click',async()=>{lastGoogleSearch=null;$('#mapSearchInput').value='';$('#mapPlaceCard').hidden=true;const map=await initGoogleMap();if(map){map.setCenter({lat:30.0444,lng:31.2357});map.setZoom(12);renderGoogleMap();}showToast('Map reset',true);});
...
function handleLocationSuccess(pos){
  ...
  userMarkerPos = { lat: pos.coords.latitude, lng: pos.coords.longitude };
